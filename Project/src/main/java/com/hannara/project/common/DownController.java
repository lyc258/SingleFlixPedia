package com.hannara.project.common;
...
public class DownController {
	...
	public void downloadFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			System.out.println("0000000000000000000000000" + fileName);
			...
			if (fSize > 0) {
				 ...
				 * BufferedInputStream
				 * 
				 * java.io의 가장 기본 파일 입출력 클래스 입력 스트림(통로)을 생성해줌 사용법은 간단하지만, 버퍼를 사용하지 않기 때문에 느림 속도
				 * 문제를 해결하기 위해 버퍼를 사용하는 다른 클래스와 같이 쓰는 경우가 많음
				 */
				 ...
				 * 
				 * java.io의 가장 기본이 되는 파일 입출력 클래스 출력 스트림(통로)을 생성해줌 사용법은 간단하지만, 버퍼를 사용하지 않기 때문에 느림
